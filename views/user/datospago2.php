...
if ($seguro == "Vehicule" || $seguro == "motorcycle" || $seguro == "Utility") {
    ...
    // Manejo del archivo subido de la tarjeta de circulación
    if (isset($_FILES['tarjeta_circulacion']) && $_FILES['tarjeta_circulacion']['error'] == UPLOAD_ERR_OK) {
        $tarjetaCirc = file_get_contents($_FILES['tarjeta_circulacion']['tmp_name']);
    ...
    $stmt1->bindParam(':numero_telefonico', $num_telBene);
    ...
    if (isset($fotos) && $fotos['error'] == UPLOAD_ERR_OK) {
        $documentacion_encoded = base64_encode(file_get_contents($fotos['tmp_name']));
    } else {
        $documentacion_encoded = null;
    }
