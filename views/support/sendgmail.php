...
$query = "SELECT id_c, nombre, correo, estado, seguro, calidad, precio, fecha_compra FROM compra_plan WHERE (fecha_compra::date + interval '30 days') <= CURRENT_DATE AND estado != 'pendiente'";
...
foreach ($compras as $compra) {
    $id = $compra['id_c'];
    $nombre = htmlspecialchars(decryptPayload($compra['nombre']));
    $correo = htmlspecialchars(decryptPayload($compra['correo']));
    ...
    $updateQuery = "UPDATE compra_plan SET estado = 'pendiente' WHERE id_c = :id";
...
if (empty($errores)) {
    echo '<script>Swal.fire({
          title: "Success",
          text: "All emails have been sent and statuses updated.",
          icon: "success",
          confirmButtonText: "OK"
          });</script>';
} else {
    echo '<script>Swal.fire({
          title: "Error",
          text: "' . count($errores) . ' emails were sent. The following errors occurred:\n' . implode("\n", $errores) . '",
          icon: "error",
          confirmButtonText: "OK"
          });</script>';
}
