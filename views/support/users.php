...
<div class="col-sm-8 col-xs-8 heading-avatar">
    <div class="heading-avatar-icon">
        <img id="imagenPerfil"> <!-- Aquí agregamos un ID para poder seleccionar fácilmente la imagen -->
...
<script>
    // Suponiendo que tengas el valor de id_tipo almacenado en una variable de PHP llamada $idTipo
    var idTipoUsuario = <?php echo $idTipoUsuario; ?>; // Asignamos el valor de PHP a una variable JavaScript

    // Seleccionamos la imagen por su ID
    var imagenPerfil = document.getElementById("imagenPerfil");

    // Lógica para cambiar la imagen según el valor de id_tipo
    if (idTipoUsuario === 2) {
        // Si id_tipo es 2, cambiamos la src de la imagen a la imagen correspondiente
        imagenPerfil.src = 'imagenesperfil/user.jpeg';
    } else if (idTipoUsuario === 3) {
        // Si id_tipo es 3, cambiamos la src de la imagen a la imagen correspondiente
        imagenPerfil.src = 'imagenesperfil/support.png';
    } else {
        ...
        if ($FilaUsers['estatus'] != 'Inactiva') {
          ...
          <div class="avatar-icon">
            ...
            <?php } elseif ($FilaUsers['id_tipo'] == 3) { ?>
              <img src="imagenesperfil/support.png" class="notification-container"
