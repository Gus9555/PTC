...
if (isset($_POST['id']) && isset($_POST['id_tipo']) && isset($_POST['action'])) {
    ...
    try {
        ...
        $sql = "SELECT email, nombre_completo FROM asociados WHERE id = :id";
        ...
        if ($user) {
            ...
            if ($action === 'accept') {
                ...
                $sql = "UPDATE asociados SET activation = 1 WHERE id = :id";
                $stmt = $pdo->prepare($sql);
                $stmt->bindParam(':id', $id, PDO::PARAM_INT);
                $stmt->execute();
                ...
                echo '<p><script>Swal.fire({
                    ...
                    window.location = "../Tabla_medico.php";
                });</script></p>';
                
            } elseif ($action === 'denied') {
                // Cambiar el estado de "activation" a 3 (denied)
                $sql = "UPDATE asociados SET activation = 3 WHERE id = :id";
                $stmt = $pdo->prepare($sql);
                $stmt->bindParam(':id', $id, PDO::PARAM_INT);
                $stmt->execute();
