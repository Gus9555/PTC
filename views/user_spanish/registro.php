        ...
        <div class="container">
            ...
            <div class="collapse navbar-collapse" id="navbarsExampleDefault">
                ...
                <ul class="navbar-nav ml-auto">
                    ...
                    <li class="nav-item">
                        <a class="nav-link page-scroll" href="../user/registro.php">ENGLISH <span
                                class="sr-only">(current)</span></a>
                    </li>
                </ul>
                <span class="nav-item">
                    <a class="btn-outline-sm" href="login.php">INICIA SESION</a>
                </span>
            </div>
        </div>
    </nav>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($nombre) || empty($usuario) || empty($password) || empty($passwordc) || empty($email) ||  empty($EspacioBlanco)) {
        echo '<p><script>Swal.fire({
            title: "ERROR!",
            text: "No dejes espacios en blanco",
            icon: "error"
        });</script></p>';
        $errors[] = "blank spaces";
        ...
        echo '<p><script>Swal.fire({
            title: "ERROR",
            text: "Direccion de correo electronico invalida",
            icon: "error"
        });</script></p>';
    } elseif (!$uppercase || !$lowercase || !$number || !$specialChars || strlen($password) < 8) {
        echo '<p><script>Swal.fire({
            title: "ERROR",
            text: "La contraseña debe incluir al menos 8 caracteres, un numero, una mayuscula y un caracter especial",
            icon: "error"
        });</script></p>';
        ...
        if ($registro > 0) {
            ...
            $asunto = 'PIN - LIFELINE';
            $cuerpo = "Querido $nombre: <br /><br />este es tu codigo de seguridad personal, no lo comparta con nadie: $codigo <br /><br /> Confirme su identidad <a href='$url'>Confirmar Correo</a>";
            //aca se envia el correo usando las anteriores mencionadas variables
            if (enviarEmail($email, $nombre, $asunto, $cuerpo)) {
                echo '<p><script>Swal.fire({
                    title: "Good job!",
                    text: "Inicio de sesion exitoso, revisa tu correo",
                    icon: "success"
                }).then(function() {
                ...
                echo '<p><script>Swal.fire({
                    ...
                    window.location = "../views/user_spanish/registro.php";
                });</script></p>';
            }
        } else {
                echo '<p><script>Swal.fire({
                    title: "ERROR",
                    text: "Ocurrio un error creando su cuenta",
                    icon: "error"
                });</script></p>';
        }
    }
